"""Split the final labeled dataset into persistent train and test CSV files."""
...
from src.models.utils import DEFAULT_LABEL_COLUMN, DEFAULT_TEXT_COLUMN, load_labeled_data


def main() -> int:
    parser = argparse.ArgumentParser(description="Split the labeled dataset into train and test files.")
    parser.add_argument(
        "--input",
        default=str(PROJECT_ROOT / "data" / "processed" / "labeled" / "final_label_relevant.csv"),
        help="Input relevant-only labeled dataset.",
    )
    parser.add_argument(
        "--text-column",
    ...
    try:
        ...
        target_column = args.label_column if args.label_column in df.columns else ("label" if "label" in df.columns else args.label_column)
        splitter = StratifiedShuffleSplit(n_splits=1, test_size=args.test_size, random_state=args.random_state)
        train_idx, test_idx = next(splitter.split(df[args.text_column].astype(str), df[target_column].astype(str)))
    ...
    print(
        ...
        + train_rows[target_column].value_counts().to_string()
    )
    print(
        "Test label distribution:\n"
        + test_rows[target_column].value_counts().to_string()
    )
    return 0
